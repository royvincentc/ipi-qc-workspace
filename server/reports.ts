...
export async function resolveReportSetup(sampleId:string):Promise<ReportSetup>{
 ...
 const searchCategory = sample.category === 'ST' ? 'FG' : sample.category;
 ...
 const productSpecs = specifications.filter(s => s.product === product.name && (s.category === sample.category || (sample.category === 'ST' && s.category === 'FG')));
 if (!safeContext) {
 ...
 const matches=type.applicability==='managed'?[{tests:[...new Set(exact.flatMap(s=>s.tests.map(t=>t.test)))]}]:resolveApplicabilityMatches(applicability, product, sample.category === 'ST' ? (managed.value.sampleTypes.find(t=>t.id==='FG')?.applicabilitySheet || type.applicabilitySheet) : type.applicabilitySheet);
 ...
 if(tests.some((t: any)=>!managed.value.tests.some(x=>x.id===t.test&&x.active&&(x.categories.includes(sample.category) || (sample.category === 'ST' && x.categories.includes('FG'))))))throw new Fault(409,'An applicable checklist test is inactive or unavailable for this sample type. Ask an administrator to review it.');
 ...
 let templates = allTemplates.filter(t=>t.category===sample.category);
 if(!templates.length) templates = allTemplates; // fallback to any category if none match the specific category
 ...
 const template=choices[0];const candidate=exact.length?[...exact].sort((a,b)=>Math.max(...b.tests.map((t: any)=>Date.parse(t.date)||0))-Math.max(...a.tests.map((t: any)=>Date.parse(t.date)||0)))[0]:{id:'blank',product:product.name,category:sample.category,context:safeContext||'Routine',revision:'0',tests:[],issues:[],source:'Spreadsheet'};const {path:_privatePath,...safeTemplate}=template;
