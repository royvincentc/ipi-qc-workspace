...
export const layoutSchema=z.object({start:z.number().int().min(0).max(100),end:z.number().int().min(0).max(100),ml:z.number().int().min(0).max(100),header:z.number().int().min(1).max(20),first:z.number().int().min(2).max(21),title:text,acceptedTitles:z.array(text).max(20).default([]),merge:z.string().regex(/^[A-Z]+\d+:[A-Z]+\d+$/),headers:z.array(z.string().max(100)).min(1).max(101),fields:z.record(z.number().int().min(0).max(100)),extraMerges:z.array(z.string().regex(/^[A-Z]+\d+:[A-Z]+\d+$/)).default([])});
